...
import { MonitorSmartphone, Cpu, Rocket, Check } from "lucide-react";
...
function Services() {
  ...
  return (
    <section id="services" className="bg-canvas py-20 relative sm:py-24 lg:py-28">
      <div className="w-[92%] lg:w-[90%] xl:w-[88%] 2xl:w-[85%] max-w-[1400px] mx-auto">
        <div className="mb-14 lg:mb-20">
          ...
          <p className="text-center text-lg px-4 lg:text-xl mt-6 lg:mt-10 text-muted font-body max-w-3xl mx-auto">
            We deliver high-quality digital solutions engineered for performance, scale and user experience.
          </p>
        </div>

     <div className="grid grid-cols-1 md:grid-cols-3 gap-6 xl:gap-6">
  {services.map((service, index) => (
    <div
      key={index}
      className="bg-panel border border-line rounded-2xl p-7 xl:p-9 flex flex-col h-full transition-colors duration-300 hover:border-primary/50"
    >
      <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-5">
        {service.icon}
      </div>

      <h3 className="font-display font-semibold text-lg xl:text-xl text-foreground mb-1.5">
        {service.title}
      </h3>
      ...
      <p className="font-body text-muted text-[14px] leading-relaxed mb-5">
        {service.description}
      </p>

      <ul className="flex flex-col gap-2 mb-7">
        {service.features.map((feature, i) => (
          <li key={i} className="flex items-center gap-2 text-[13px] text-muted font-body">
      ...
      <a href="/#contact" className="text-center mt-auto w-full font-nav text-[13.5px] font-semibold tracking-[0.02em] py-2.5 rounded-full bg-primary text-white opacity-90 hover:opacity-100 transition-opacity duration-200">
        Get Started
      </a>
    </div>
  ))}
